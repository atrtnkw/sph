...
!      include 'network.dek'
!
!! this program exercises the iso7 network
!
!! declare
!      integer          i,j,nok,nbad
!      double precision time_start,time_end,dtsav,conserv,tin,din,ein,vin,zin,xin(7), &
...
! for the integration driver
      ...
      external         iso7,siso7,biso7,diso7
...
!                  ymass(ionbeg),abar,zbar,wbar,ye,xcess)
       call azbariso7(xmass(ionbeg),aion(ionbeg),zion(ionbeg),wion(ionbeg),ionmax, &
                  ymass(ionbeg),abar,zbar,wbar,ye,xcess)
...
! integrate the iso7 network
       call netint(time_start,stptry,stpmin,time_end,dtsav,ys2, &
                   tol,neqs,nok,nbad,kount,odescal, &
!                  iso7,siso7,biso7,forder_ma28)
!                  iso7,siso7,biso7,forder_umf)
!                  iso7,siso7,biso7,forder_y12m)
!                  iso7,diso7,biso7,forder_ludcmp)
!                  iso7,diso7,biso7,forder_leqs)
!                  iso7,diso7,biso7,forder_lapack)
!                  iso7,diso7,biso7,forder_gift)
!                  iso7,siso7,biso7,forder_biconj)
!                  iso7,siso7,biso7,rosen_ma28)
!                  iso7,siso7,biso7,rosen_umf)
!                  iso7,siso7,biso7,rosen_y12m)
!                  iso7,diso7,biso7,rosen_ludcmp)
!                  iso7,diso7,biso7,rosen_leqs)
!                  iso7,diso7,biso7,rosen_lapack)
!                  iso7,diso7,biso7,rosen_gift)
!                  iso7,siso7,biso7,rosen_biconj)
                  iso7,siso7,biso7,stifbs_ma28)
!                  iso7,siso7,biso7,stifbs_umf)
!                  iso7,siso7,biso7,stifbs_y12m)
!                  iso7,diso7,biso7,stifbs_ludcmp)
!                  iso7,diso7,biso7,stifbs_leqs)
!                  iso7,diso7,biso7,stifbs_lapack)
!                  iso7,diso7,biso7,stifbs_gift)
!                  iso7,siso7,biso7,stifbs_biconj)
...
! routine init_iso7 initializes the iso7 network
...
! local variables
      integer          i
      double precision enuc,taud,taut,sum1,sum2,z, &
                       abar,zbar,ye,snuda,snudz
...
! 4he reactions
      dydt(ihe4) =  3.0d0 * y(ic12) * rate(irg3a) &
                  ...
                  + 0.5d0 * y(io16) * y(io16) * rate(ir1616) &
                  ...
                  - 7.0e0 * rate(irsi2ni) * y(ihe4) &
                  + 7.0e0 * rate(irni2si) * y(ini56)


! 12c reactions
      dydt(ic12) =   sixth * y(ihe4) * y(ihe4) * y(ihe4) * rate(ir3a) &
                   - y(ic12) * rate(irg3a) &
                   + y(io16) * rate(iroga) &
                   - y(ic12) * y(ihe4) * rate(ircag) &
                   - y(ic12) * y(ic12) * rate(ir1212) &
...
! 28si reactions
      dydt(isi28) =  0.5d0 * y(ic12) * y(io16) * rate(ir1216) &
                   ...
                   + y(img24) * y(ihe4) * rate(irmgag) &
                   - rate(irsi2ni) * y(ihe4) &
                   + rate(irni2si) * y(ini56)

! ni56 reactions
      dydt(ini56) =  rate(irsi2ni) * y(ihe4) &
                   - rate(irni2si) * y(ini56)
...
! local variables
      integer          i,j
      double precision abar,zbar,ye,taud,taut, &
                       snuda,snudz,sum1,sum2,xx,zz
...
! 4he jacobian elementss
      dfdy(ihe4,ihe4)  = -1.5d0 * y(ihe4) * y(ihe4) * ratdum(ir3a) &
                        ...
                        - 7.0d0 * ratdum(irsi2ni) &
                        - 7.0d0 * dratdumdy1(irsi2ni) * y(ihe4) &
                        + 7.0d0 * dratdumdy1(irni2si) * y(ini56)

      dfdy(ihe4,ic12)  =  3.0d0 * ratdum(irg3a) &
                        - y(ihe4) * ratdum(ircag) &
                        + y(ic12) * ratdum(ir1212) &
                        + 0.5d0 * y(io16) * ratdum(ir1216)

      dfdy(ihe4,io16)  =  ratdum(iroga) &
                        + 0.5d0 * y(ic12) * ratdum(ir1216) &
                        + y(io16) * ratdum(ir1616) &
                        - y(ihe4) * ratdum(iroag)
      ...
      dfdy(ihe4,isi28) =  ratdum(irsiga) &
                        - 7.0d0 * dratdumdy2(irsi2ni) * y(ihe4)

      dfdy(ihe4,ini56) =  7.0d0 * ratdum(irni2si)
...
! 28si jacobian elements
      dfdy(isi28,ihe4)  =  y(img24) * ratdum(irmgag) &
                         - ratdum(irsi2ni) &
                         - dratdumdy1(irsi2ni) * y(ihe4) &
                         + dratdumdy1(irni2si) * y(ini56)

      dfdy(isi28,ic12)  = 0.5d0 * y(io16) * ratdum(ir1216)

      dfdy(isi28,io16)  =  y(io16) * ratdum(ir1616) &
                         + 0.5d0 * y(ic12) * ratdum(ir1216)

      dfdy(isi28,img24) =  y(ihe4) * ratdum(irmgag)

      dfdy(isi28,isi28) = -ratdum(irsiga) &
                         - dratdumdy2(irsi2ni) * y(ihe4)

      dfdy(isi28,ini56) =  ratdum(irni2si)



! ni56 jacobian elements
      dfdy(ini56,ihe4)  =   ratdum(irsi2ni) &
                          + dratdumdy1(irsi2ni) * y(ihe4) &
                          - dratdumdy1(irni2si) * y(ini56)

      dfdy(ini56,isi28) =  dratdumdy2(irsi2ni) * y(ihe4)

      dfdy(ini56,ini56) = -ratdum(irni2si)
...
! append the energy generation rate jacobian elements
      do j=1,ionmax
       ...
       enddo
       dfdy(iener,j) = dfdy(iener,j) * enuc_conv
       dfdy(iener,itemp) = dfdy(iener,itemp) + dfdy(j,itemp)*bion(j)
       dfdy(iener,iden)  = dfdy(iener,iden) + dfdy(j,iden)*bion(j)
      enddo
      dfdy(iener,itemp) = dfdy(iener,itemp) * enuc_conv
      dfdy(iener,iden)  = dfdy(iener,iden) * enuc_conv
...
! shut down the temperature and density derivatives
!      do i=1,ionmax
!       dfdy(i,itemp) = 0.0d0
!       dfdy(i,iden) = 0.0d0
!      enddo
!      dfdy(iener,itemp) = 0.0d0
!      dfdy(iener,iden) = 0.0d0
...
! 28si jacobian elements
      ...
      call tree(isi28,ini56,eloc,neloc,nterms,nzo,iloc,jloc,np)

! ni56 jacobian elements
      call tree(ini56,ihe4,eloc,neloc,nterms,nzo,iloc,jloc,np)
      call tree(ini56,isi28,eloc,neloc,nterms,nzo,iloc,jloc,np)
...
! local variables
      ...
      integer            :: i,nt,iat
      real*8             :: zz,xx,a1,a2,a3,a4, & 
                            abar,zbar,z2bar,taud,taut,snuda,snudz,ye,enucbt,enucbd
      real*8             :: xsum(abignet)
...
! d(he4)/d(he4)
      a1 = -1.5d0 * y(ihe4) * y(ihe4) * ratdum(ir3a) &
          - y(ic12) * ratdum(ircag) &
          - y(io16) * ratdum(iroag) &
          - y(ine20) * ratdum(irneag) &
          - y(img24) * ratdum(irmgag) &
      ...
      xsum(ihe4) = xsum(ihe4) + a1 * bion(ihe4)

! d(he4)/d(c12)
      a1 =  3.0d0 * ratdum(irg3a) &
          - y(ihe4) * ratdum(ircag) &
          + y(ic12) * ratdum(ir1212) &
          + 0.5d0 * y(io16) * ratdum(ir1216)
      nt  = nt + 1
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(ic12) = xsum(ic12) + a1 * bion(ihe4)
...
! d(he4)/d(o16)
      ...
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(io16) = xsum(io16) + a1 * bion(ihe4)
...
! d(he4)/d(ne20)
      ...
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(ine20) = xsum(ine20) + a1 * bion(ihe4)
...
! d(he4)/d(mg24)
      ...
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(img24) = xsum(img24) + a1 * bion(ihe4)
...
! d(he4)/d(si28)
      ...
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(isi28) = xsum(isi28) + a1 * bion(ihe4)

! d(he4)/d(ni56)
      a1 =  7.0d0 * ratdum(irni2si)
      nt  = nt + 1
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(ini56) = xsum(ini56) + a1 * bion(ihe4)



! 12c jacobian elements
! d(c12)/d(he4)
      a1 =  0.5d0 * y(ihe4) * y(ihe4) * ratdum(ir3a) &
          - y(ic12) * ratdum(ircag)
      nt  = nt + 1
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(ihe4) = xsum(ihe4) + a1 * bion(ic12)
...
! d(o16)/d(he4)
      ...
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(ihe4) = xsum(ihe4) + a1 * bion(io16)
...
! d(o16)/d(ne20)
      ...
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(ine20) = xsum(ine20) + a1 * bion(io16)
...
! d(ne20)/d(he4)
      ...
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(ihe4) = xsum(ihe4) + a1 * bion(ine20)
...
! d(ne20)/d(o16)
      ...
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(io16) = xsum(io16) + a1 * bion(ine20)
...
! d(ne20)/d(mg24)
      ...
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(img24) = xsum(img24) + a1 * bion(ine20)
...
! d(mg24)/d(he4)
      ...
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(ihe4) = xsum(ihe4) + a1 * bion(img24)
...
! d(mg24)/d(ne20)
      ...
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(ine20) = xsum(ine20) + a1 * bion(img24)
...
! d(mg24)/d(si28)
      ...
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(isi28) = xsum(isi28) + a1 * bion(img24)
...
! d(si28)/d(he4)
      ...
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(ihe4) = xsum(ihe4) + a1 * bion(isi28)
...
! d(si28)/d(c12)
      ...
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(ic12) = xsum(ic12) + a1 * bion(isi28)

! d(si28)/d(o16)
      a1 =  y(io16) * ratdum(ir1616) &
          + 0.5d0 * y(ic12) * ratdum(ir1216)
      nt  = nt + 1
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
...
! d(si28)/d(mg24)
      ...
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(img24) = xsum(img24) + a1 * bion(isi28)
...
! d(si28)/d(ni56)
      ...
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(ini56) = xsum(ini56) + a1 * bion(isi28)
...
! d(ni56)/d(he4)
      ...
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(ihe4) = xsum(ihe4) + a1 * bion(ini56)

! d(ni56)/d(si28)
      a1 = dratdumdy2(irsi2ni) * y(ihe4)
      nt  = nt + 1
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(isi28) = xsum(isi28) + a1 * bion(ini56)
...
! d(ni56)/d(ni56)
      ...
      iat = eloc(nt)
      dfdy(iat) = dfdy(iat) + a1
      xsum(ini56) = xsum(ini56) + a1 * bion(ini56)
...
! temp depend of partition functions
!      gca40=1.0d0 +exp((-4.150E+01 + 1.636E+00*t9 + 1.483E-01*t92)*t9ri)
!      gti44=1.0d0 +exp((-1.111E+01 + 6.293E-01*t9 + 1.732E-01*t92)*t9ri)
!      ratraw(irtiga) = ratraw(irtiga) * gca40/gti44
...
!      ratraw(irmgag) = 0.0d0
!      ratraw(irsiga) = 0.0
!      ratraw(ircaag) = 1.0d-40
!      ratraw(irtiga) = 1.0d-40
...
! since some of iso7's effective reaction rates are
! determined in routine weak_iso7, we need set the correct
! temperature factors
...
! this routine assumes screen_on = 1 or = 0 has been set at a higer level,
! presumably in the top level driver
...
! declare
      ...
      double precision y(*),sc1a,sc1adt,sc1add,sc2a,sc2adt,sc2add, &
                       ...
                       t992,t9i92,denom,zz, &
                       yeff_ca40,yeff_ca40dt,yeff_ti44,yeff_ti44dt
...
! form those lovely dummy links

! rsi2ni is the rate for silicon to nickel
! rni2si is the rate for nickel to silicon
      ratdum(irsi2ni)     = 0.0d0
      dratdumdy1(irsi2ni) = 0.0d0
      dratdumdy2(irsi2ni) = 0.0d0
      dratdumdt(irsi2ni)  = 0.0d0
      dratdumdd(irsi2ni)  = 0.0d0

      ratdum(irni2si)     = 0.0d0
...
! oxygen burning is gone.
      ...
      if (t9 .gt. 2.5 .and. y(ic12)+y(io16) .le. 4.0d-3) then
       ...
       yeff_ca40dt = -yeff_ca40*(239.42*t9i2 + 4.5d0*t9i)

       yeff_ti44   = t992  * exp(-274.12*t9i + 74.914)
       yeff_ti44dt = yeff_ti44*(274.12*t9i2 + 4.5d0*t9i)
...
! set the names of the reaction rates
      ...
      ratnam(irsi2ni) = 'rsi2ni'
      ratnam(irni2si) = 'rni2si'
...
!                   zwork1,abar,zbar,wbar,ye_orig,xcess)
        call azbariso7(xin,aion,zion,wion,ionmax, &
                   zwork1,abar,zbar,wbar,ye_orig,xcess)
...
!                  zwork1(ionbeg),abar,zbar,wbar,yex,xcess)
       call azbariso7(xdum(ionbeg),aion(ionbeg),zion(ionbeg),wion(ionbeg),ionmax, &
                  zwork1(ionbeg),abar,zbar,wbar,yex,xcess)
...
!      call azbar(zwork2,aion,zion,wion,ionmax, &
!                 zwork1,abar,zbar,wbar,ye,xc)
      call azbariso7(zwork2,aion,zion,wion,ionmax, &
                 zwork1,abar,zbar,wbar,ye,xc)
...
!                 ymass(ionbeg),abar,zbar,wbar,ye,xcess)
      call azbariso7(xmass_up(ionbeg),aion(ionbeg),zion(ionbeg),wion(ionbeg),ionmax, &
                 ymass(ionbeg),abar,zbar,wbar,ye,xcess)
...
!                       ymass,abar,zbar,wbar,ye,nxcess)
      subroutine azbariso7(xmass,aion,zion,wion,ionmax, &
                       ymass,abar,zbar,wbar,ye,nxcess)
      include 'implno.dek'
